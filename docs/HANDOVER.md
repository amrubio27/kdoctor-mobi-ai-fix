...
**Última actualización**: Round-3 #12 y #16 implementados y validados ✅. **Round-3 release prep COMPLETO**: #8, #9, #9.1, #9.2, #10, #11, #12, #13, #14, #15, #16 cerrados. Pendiente: v1.0.0 release notes / publicación. Tag `v1.0.0` sigue apuntando a `bfe5d46` (HEAD está varios commits adelante con dashboard/Gradle/K2-FIR no documentado en handover); decidir si se mueve al commit actual o se publica como v1.0.1.
...
| **10** | README.md rewrite para público | ✅ cerrado | README.md reescrito en inglés para audiencia pública: hero con badges (CI, Go version, License), tagline, feature grid (8 items), quickstart de 3 pasos (install/init/scan), secciones de instalación (binary/go install/source), uso detallado de `scan`, `fix --ai`, `rules`, y dashboard HTML; tabla de flags del scan; ejemplo de configuración `kdoctor.config.yaml` con cluster/rule overrides y excludes; CI/GitHub Actions snippet usando `.github/workflows/ci.yml`; fórmula del Health Score; tabla comparativa kdoctor vs Detekt vs Android Lint; roadmap actualizado; sección de contributing con validation checklist; licencia MIT. Se evitó sobre-prometer: `fix --mode auto` se etiqueta como "still being hardened for v1.0.1"; el feature "Zero JVM setup" se cambió a "Single Go binary" con nota de que Detekt/Gradle aún requieren JVM. Validación: `go build`, `./kdoctor --version`, `./kdoctor --help`, `./kdoctor scan --help`, `./kdoctor rules` funcionan correctamente; `gofmt -l .` limpio. |
...
| **11** | Tier 2 #5 `kdoctor fix --mode auto` (patch apply + rollback) | ✅ cerrado | Implementado en `internal/cli/fix.go` con `applyFix`: extrae bloque ` ```kotlin ` via `internal/aifixer/applier.ExtractCodeBlock`, reemplaza la ventana de líneas mostrada a la LLM (usando `qualityprompt.SliceRange`/`SplitLines`), valida el archivo resultante con `patchguard.Validate`, y sólo escribe si pasa; fallo mantiene archivo original. Tests: `internal/aifixer/applier/applier_test.go` (6 cases) + `internal/cli/fix_test.go` (3 cases: applied, rollback on invalid, empty patch). `qualityprompt.BuildPromptWithContext` ahora instruye a la LLM a devolver el bloque completo. Modo `suggest` sigue generando `fixes.md` sin tocar código. |
| **12** | Tests E2E con proyecto Android real | ✅ cerrado | Nuevo runner `scripts/e2e-rick-morty/main.go` con flag `--smoke`. Valida ejecución de `kdoctor scan --json` contra `RICK_MORTY_APP` (default `D:/Programacion/RickMortyApp`), schema version, score en rango [0,100], y presencia de `arch-god-class`. Test `TestSmokeRickMortyApp` en `scripts/e2e-rick-morty/main_test.go` hace SKIP cuando el proyecto no está disponible (CI-friendly). Lee `KDOCTOR_BIN` / `KDOCTOR_DETEKT_BIN` / `RICK_MORTY_APP` para configuración. READ-ONLY sobre el proyecto del usuario. |
| **13** | `make` build targets + Dockerfile | ✅ cerrado | Makefile añadido con targets: `build`, `test`, `test-race`, `vet`, `fmt`, `fmt-check`, `tidy`, `smoke`, `e2e-rick-morty`, `doctor`, `rules`, `clean`, `dashboard-build`, `dashboard-dev`, `lint`, `ci`, `help`. `smoke` usa `go test -race -run TestEval ./scripts/evalprojects/...` para ser robusto ante la ubicación de detekt. `ci` corre `lint test-race build smoke`. Dockerfile multi-stage: `golang:1.23-alpine` builder → descarga de `detekt-cli-1.23.8-all.jar` → runtime `eclipse-temurin:17-jre-alpine`. `.dockerignore` añadido para excluir `.git`, `dashboard/node_modules`, binarios, docs, etc. README.md actualizado con secciones "Build with Make" y "Docker". |
| **14** | `kdoctor init` project bootstrapper | ✅ cerrado | `internal/cli/init.go` reescrito como bootstrapper project-aware. `config.ForProjectType(type)` genera configs adaptadas a `android`, `kmp`, `cmp`, `compose`, `jvm`, `gradle`, `plain`. Detección heurística por directorios (`composeApp`, `commonMain`, `src/main/kotlin`), plugins Gradle (`com.android.application`, `org.jetbrains.compose`, `kotlin("multiplatform")`), y manifiesto Android. Genera `kdoctor.config.yaml`, `detekt.yml` con V1 live rules activas, y actualiza `.gitignore` con entradas de kdoctor. Tests en `internal/cli/init_test.go` cubren detección (CMP/KMP/Android/JVM/Gradle/Plain), creación de archivos, rechazo de sobrescritura, `--force`, e idempotencia de `.gitignore`. README actualizado con sección `kdoctor init`. |
| **15** | Release notes v1.0 + semver tag | ✅ cerrado | Round-3 closes con `git tag v1.0.0`, `CHANGELOG.md` con resumen de lo cerrado en tiers 1-3 + round-2-3, `CHANGELOG.md` actualizado con Round-3 release prep; tag `v1.0.0` apunta a HEAD (`187b9ec`). |
| **16** | MobiAI Graph integration end-to-end | ✅ cerrado | Nuevo paquete `internal/mobiai` con `Client` que hace POST a `{baseURL}/graph/findings` con JSON payload `{tool,projectPath,findings[]}`. Flags CLI: `--mobiai-url` / `--mobiai-token` / `--mobiai-fail-on-error`; también variables de entorno `KDOCTOR_MOBIAI_URL` y `KDOCTOR_MOBIAI_TOKEN`. Si `--mobiai` se usa sin URL, conserva el comportamiento previo (escritura local de `.mobiai/graph/findings.jsonl`). Si la subida falla y `--mobiai-fail-on-error` no está activo, solo se emite warning a stderr. Tests en `internal/mobiai/client_test.go` con `httptest`. |
