...
# kdoctor runs detekt with --build-upon-default-config, so this file LAYERS
# on top of detekt's bundled defaults instead of replacing them: every
# default rule stays active and the entries below only adjust what they say.
# Note this differs from running detekt-cli --config yourself, where the
# file does replace the defaults.
...
naming:
  ...
  FunctionNaming:
    ...
    ignoreAnnotated:
      - "Composable"
    # Kotlin test functions are conventionally backticked sentences.
    # Without these excludes a typical KMP project reports hundreds of
    # FunctionNaming violations that bury every real finding.
    excludes:
      - '**/test/**'
      - '**/androidTest/**'
      - '**/androidHostTest/**'
